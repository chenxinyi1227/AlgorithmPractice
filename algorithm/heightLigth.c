...
/* 
在上述代码中，`\033[`是ANSI控制字符的起始符号，后面的数字表示不同的颜色和高亮选项，
`m`是ANSI控制字符的结束符号。在每个文本输出结束后，需要使用`\033[0m`将高亮选项重置为默认值，以免影响后续的文本输出。
需要注意的是，ANSI控制字符只在支持ANSI控制序列的终端中起作用，
如常见的Linux终端、macOS终端等。在Windows环境下，可能需要特定的库或工具才能实现类似的高亮效果。 */